...
install_miniconda()
{
    ...
    if [ -d "$HOME/miniconda3" ] && [ -f "$HOME/miniconda3/bin/conda" ]; then
        print_message "检测到Miniconda已安装，跳过安装步骤"
        return 0
    fi
    ...
    if wget -O Miniconda3-latest.sh "$MINICONDA_URL"; then
        print_message "下载完成"
    else
        print_error "下载失败，尝试使用curl..."
        if curl -L -o Miniconda3-latest.sh "$MINICONDA_URL"; then
            print_message "下载完成"
        else
    ...
    if bash Miniconda3-latest.sh -b -p "$HOME/miniconda3"; then
...
install_mamba()
{
    ...
    # 尝试使用conda安装mamba
    print_message "尝试使用conda安装mamba..."
    if "$HOME/miniconda3/bin/conda" install mamba -n base -c conda-forge -y; then
        print_message "mamba安装成功"
        return 0
    else
        print_warning "conda安装mamba失败，尝试使用pip安装..."

        # 使用pip安装mamba
        if "$HOME/miniconda3/bin/pip" install mamba; then
            print_message "mamba安装成功（通过pip）"
            return 0
        else
            print_error "mamba安装失败"
            return 1
        fi
    fi
}
...
configure_pip_mirrors()
{
    ...
    # 设置阿里云镜像源
    print_message "设置阿里云PyPI镜像源..."
    "$HOME/miniconda3/bin/pip" config set global.index-url https://mirrors.aliyun.com/pypi/simple/
    "$HOME/miniconda3/bin/pip" config set global.trusted-host mirrors.aliyun.com

    # 验证配置
    print_message "验证pip配置..."
    "$HOME/miniconda3/bin/pip" config list

    print_message "pip镜像源配置完成"
}
...
verify_installation()
{
    ...
    if [ -f "$HOME/miniconda3/bin/conda" ]; then
        print_message "✓ conda安装成功: $($HOME/miniconda3/bin/conda --version)"
    else
        print_error "✗ conda安装失败"
    fi

    # 检查mamba
    if [ -f "$HOME/miniconda3/bin/mamba" ]; then
        print_message "✓ mamba安装成功: $($HOME/miniconda3/bin/mamba --version)"
    else
        print_error "✗ mamba安装失败"
    fi

    # 检查pip配置
    if "$HOME/miniconda3/bin/pip" config list 2>/dev/null | grep -q "mirrors.aliyun.com"; then
        print_message "✓ pip镜像源配置成功"
    else
        print_error "✗ pip镜像源配置失败"
    fi
}
...
show_usage()
{
    print_message "安装完成！使用说明："
    echo ""
    echo "1. 重新打开终端或运行: source ~/.bashrc"
    echo "2. 使用conda管理环境: conda create -n myenv python=3.9"
    echo "3. 使用mamba安装包（更快）: mamba install numpy"
    echo "4. 使用pip安装Python包: pip install requests"
    echo ""
    echo "镜像源配置："
    echo "- conda: 清华大学镜像源"
    echo "- pip: 阿里云镜像源"
